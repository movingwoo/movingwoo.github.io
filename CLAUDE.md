...
- **Liquid `{% include %}`은 같은 파일을 자기 자신으로 재귀 호출하면 안 됨**: 카테고리 트리처럼 깊이가 가변적인 구조를 Liquid 재귀 include로 만들면, 이 프로젝트의 Jekyll/Liquid 버전에서 파서가 재진입을 제대로 처리하지 못해 전혀 무관한 파일·라인에서 가짜 `Liquid::SyntaxError`가 발생함 (디버깅 시 에러 메시지를 신뢰하면 안 됨). 트리/재귀 구조가 필요하면 `_plugins/`에 커스텀 Liquid 태그(Ruby)를 만들어 Ruby 메서드 재귀로 처리할 것 — 예시: `_plugins/toc_tree_tag.rb`
